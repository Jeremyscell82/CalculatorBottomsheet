...
//        resizeCreatedView(view, dialog!!, activity as MainActivity)
        calc = CalculatorImpl(formulaView = calc_formula, numberView = calc_result, context = view.context)
        view.apply {
            ...
            calc_copy.setOnClickListener { copyToClipboard(calc_result.text.toString()) }
    ...
    private fun copyToClipboard(value: String): Boolean {
        return if (value.isEmpty()){
            ...
            ((activity as MainActivity).getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(clip)
